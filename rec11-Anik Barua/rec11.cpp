/*
    Name: Anik Barua
    Subject: rec11.cpp - Test code for students to use for the linked list basics recitation.
    Date: 04-15-2022
*/
...
const Node* isSublist(const Node* pattern, const Node* target) {
    ...
    while(target != nullptr && pattern != nullptr ) {
        target = targetStart;
        while(target != nullptr && pattern != nullptr && target->data == pattern->data ){
            target = target->next;
...
const Node* sharedListBruteForce(const Node* listA, const Node* listB) {
    const Node* listAstart = listA;
    const Node* listBstart = listB;
    while(listAstart != nullptr){
        listBstart = listB;
        while(listBstart != nullptr) {
            if(listAstart == listBstart) {
                return listAstart;
            } else {
                listBstart = listBstart->next;
            }
        }
        listAstart = listAstart->next;
    }
    return nullptr;
}
